...
function hashBlake2b(input) {
  ...
  var ctx = (0, _blakejs.blake2bInit)(outlen, view);
  return Buffer.from((0, _blakejs.blake2bFinal)(ctx));
}

function hashAll(items) {
  return hashBlake2b(items.reduce(function (encItems, item) {
...
function generateKeypairDeterministic(entropy) {
  ...
  var rawPrivateKey = hashBlake2b(entropy).toString('hex');
  var kp = Ed25519.keyFromSecret(rawPrivateKey);
  var publicKey = Buffer.from(kp.pubBytes());
  var privateKey = Buffer.concat([Buffer.from(kp.privBytes()), publicKey]);
